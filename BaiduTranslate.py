...
def baidu(txt, from_language='zh', to_language='en'):
    d = Dict()
    json = d.dictionary(txt, dst=to_language, src=from_language)
    return json['trans_result']['data'][0]['dst']


def deep(ans, from_language='zh', to_language='en'):
        words = ans['trans_result']['data'][0]['dst']
        sim = {i['means'][0]:i['means'][1:-1] for i in ans['trans_result']['keywords']}
        randindex = {i:randint(-1,len(sim[i]) - 1) for i in sim}
        for i in sim:
            if randindex[i] != -1:
                words = words.replace(i,sim[i][randindex[i]])
        return words

def baidudeep(txt, from_language='zh', to_language='en'):
    d = Dict()
    json = d.dictionary(txt, dst=to_language, src=from_language)
    return deep(json,from_language=from_language, to_language=to_language)
